<template>
  <div class="add-address">
    <head-top head-title="新增地址" :go-back="true" :sign-up="true"></head-top>
...
export default {
    ...
    computed: {
        ...mapState(['geohash', 'userInfo', 'searchAddress'])
    },
    methods: {
        ...mapMutations(['RECORD_NEW_ADDRESS']),
        //保存地址信息
        addAddress(){
            ...
            addNewAddress(this.userInfo.user_id, this.searchAddress.name, this.address_detail, this.geohash, this.name, this.phone, this.anntherPhoneNumber, 0, this.sex, this.tag, this.tag_type)
                .then(res => {
                    //保存成功返沪上一页，否则弹出提示框
                    if (res.message) {
                        this.showAlert = true;
